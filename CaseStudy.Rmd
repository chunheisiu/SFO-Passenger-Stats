...
```{r top5_domestic_area}
transparent <- 0.9

data %>% filter(isDomestic) %>% group_by(year) %>% 
  summarise(domestic_pax=sum(pax)/1000000,
            domestic_ua=sum(ifelse(code=="UA",pax,0)/1000000),
            domestic_aa=sum(ifelse(code=="AA",pax,0)/1000000),
            domestic_dl=sum(ifelse(code=="DL",pax,0)/1000000),
            domestic_vx=sum(ifelse(code=="VX",pax,0)/1000000),
            domestic_wn=sum(ifelse(code=="WN",pax,0)/1000000)
            ) %>% 
  ggplot(aes(x=year)) + geom_area(aes(y=domestic_pax,fill="domestic_pax"),alpha=transparent) + 
  geom_area(aes(y=domestic_wn+domestic_vx+domestic_dl+domestic_aa+domestic_ua,fill="domestic_wn+domestic_vx+domestic_dl+domestic_aa+domestic_ua"),alpha=transparent) + 
  geom_area(aes(y=domestic_vx+domestic_dl+domestic_aa+domestic_ua,fill="domestic_vx+domestic_dl+domestic_aa+domestic_ua"),alpha=transparent) +
  geom_area(aes(y=domestic_dl+domestic_aa+domestic_ua,fill="domestic_dl+domestic_aa+domestic_ua"),alpha=transparent) +
  geom_area(aes(y=domestic_aa+domestic_ua,fill="domestic_aa+domestic_ua"),alpha=transparent) +
  geom_area(aes(y=domestic_ua,fill="domestic_ua"),alpha=transparent) +
  scale_fill_discrete(name = "Airline",label=c("American Airlines","Delta Airlines","Other Airlines","United Airlines","Virgin America","Southwest Airlines")) + scale_x_continuous(name = "Year", breaks = seq(2006, 2017, by = 1)) + 
  scale_y_continuous(name= "Passengers (Millions)", labels = comma) + theme_minimal() +
  ggtitle("Passengers Count by Airlines") + theme(plot.title = element_text(hjust = 0.5)) 
  
```

```{r top_international_list}
...
```{r top5_intl_area}
data %>% filter(!isDomestic) %>% group_by(year) %>% 
  summarise(intl_pax=sum(pax)/1000000,
            intl_ua=sum(ifelse(code=="UA",pax,0)/1000000),
            intl_ca=sum(ifelse(code=="CA",pax,0)/1000000),
            intl_lh=sum(ifelse(code=="LH",pax,0)/1000000),
            intl_cx=sum(ifelse(code=="CX",pax,0)/1000000),
            intl_ba=sum(ifelse(code=="BA",pax,0)/1000000)
            ) %>% 
  ggplot(aes(x=year)) + geom_area(aes(y=intl_pax,fill="red"),alpha=transparent) + 
  geom_area(aes(y=intl_ua+intl_ca+intl_lh+intl_cx+intl_ba,fill="green"),alpha=transparent) + 
  geom_area(aes(y=intl_ua+intl_ca+intl_lh+intl_cx,fill="blue"),alpha=transparent) +
  geom_area(aes(y=intl_ua+intl_ca+intl_lh,fill="pink"),alpha=transparent) +
  geom_area(aes(y=intl_ua+intl_ca,fill="black"),alpha=transparent) +
  geom_area(aes(y=intl_ua,fill="orange"),alpha=transparent) +
  scale_fill_discrete(name = "Airline",label=c("Air Canada","Cathay Pacific","British Airways", "United Airlines","Luthansa","Other Airlines")) + scale_x_continuous(name = "Year", breaks = seq(2006, 2017, by = 1)) + 
  scale_y_continuous(name= "Passengers (Millions)", labels = comma) + theme_minimal() +
  ggtitle("Passengers Count by Airlines") + theme(plot.title = element_text(hjust = 0.5)) 
...
```{r terminal_domestic}
data %>% filter(isDomestic) %>% group_by(terminal,airline,code) %>% summarise(all_pax=sum(pax)) %>% 
  ggplot(aes(area=all_pax,fill=terminal,label=code,group = airline))+geom_treemap()+geom_treemap_text(
    colour = "white",
    place = "centre"
  )
```
...
data %>% filter(!isDomestic & region!="Canada") %>% group_by(terminal,airline,code) %>% summarise(all_pax=sum(pax)) %>% 
  ggplot(aes(area=all_pax,fill=terminal,label=code,group = airline))+geom_treemap()+geom_treemap_text(
    colour = "white",
    place = "centre"
  )
...
```{r terminal}
data %>% group_by(terminal,airline,code) %>% summarise(all_pax=sum(pax)) %>% 
  ggplot(aes(area=all_pax,fill=terminal,label=code,group = airline))+geom_treemap()+geom_treemap_text(
    colour = "white",
    place = "centre"
  )
...
```{r united}
data %>% filter(code=="UA") %>% group_by(month,year) %>% summarise(Passengers=sum(pax)) %>% ggplot(aes(x=month,y=year))+geom_tile(aes(fill=Passengers))+ theme_classic() + scale_fill_gradient(low="red",high="green") + scale_y_continuous(expand = c(0,0), name= "Year", breaks = seq(2006, 2017, by = 1)) + scale_x_discrete(name="Month",labels = c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sept","Oct","Nov","Dec")) + ggtitle("United Airlines Passengers Count") + theme(plot.title = element_text(hjust = 0.5))
```

```{r lufthansa}
data %>% filter(code=="CX") %>% group_by(month,year) %>% summarise(Passengers=sum(pax)) %>% ggplot(aes(x=month,y=year))+geom_tile(aes(fill=Passengers))+ theme_classic() + scale_fill_gradient(low="red",high="green") + scale_y_continuous(expand = c(0,0), name= "Year", breaks = seq(2006, 2017, by = 1)) + scale_x_discrete(name="Month",labels = c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sept","Oct","Nov","Dec")) + ggtitle("Because Airlines Passengers Count") + theme(plot.title = element_text(hjust = 0.5))
```
