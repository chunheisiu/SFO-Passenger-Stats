...
```{r average_month_pax}
data %>% group_by(isDomestic,month) %>% summarise(avg_pax = mean(pax)) %>% 
  ggplot(aes(x=month,y=avg_pax,fill=isDomestic)) + theme_minimal() +  geom_bar(stat="identity",alpha=0.8) + labs(x="Month",y="Passengers") + scale_y_continuous(labels = comma) + 
