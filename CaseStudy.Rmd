...
world %>% 
  ...
  geom_curve(data = cities %>% filter(type == "Deplaned"), aes(x = long, y = lat, size = log(pax)),
             xend = sf$long, yend = sf$lat, color = "dodgerblue2", curvature = 0.5) +
  geom_curve(data = cities %>% filter(type == "Enplaned"), aes(x = long, y = lat, size = log(pax)),
             xend = sf$long, yend = sf$lat, color = "mediumseagreen", curvature = -0.5) +
  geom_point(x = sf$long, y = sf$lat, color = "red", size = 3, shape = 10) +
