...
## Read the data
data <- read_csv("Air_Traffic_Passenger_Statistics.csv")
```
...
other_dom_airline <- data %>% 
  filter(!(airline %in% top5_domestic_list$airline)) %>% 
  group_by(year) %>% 
...
data %>%
  ...
  right_join(top5_domestic_list, by = "airline") %>%
  ungroup() %>% 
  rbind(other_dom_airline) %>%
  mutate(airline = factor(airline, levels = rbind(top5_domestic_list, "Other Airlines")$airline)) %>% 
  ggplot() +
  ...
  scale_fill_brewer(palette = "Set2") +
  theme_minimal() +
  ggtitle("Passengers Count by Airlines") +
  format_title
