...
data %>%
  ...
  ggplot(aes(x = factor(month, labels = month.abb), y = avg_pax, fill = isDomestic, label = avg_pax)) +
  theme_minimal() +
...
data %>%
  ...
  coord_polar(theta = "y") +
  theme_void() +
  geom_text(aes(x = 1, y = cumsum(traffic) - traffic / 2,
                label = paste(100 * round(traffic / total_traffic[1,1], digit = 4), "%", sep = ""))) +
  scale_fill_brewer(palette = "Set2", name = "", label = c("Arrival", "Departure", "Transit")) +
  ggtitle("Airplane Activities") +
...
data %>%
  ...
  ggplot(aes(x = year)) +
  geom_area(aes(y = domestic_pax, fill = "domestic_pax"), alpha=transparent) + 
