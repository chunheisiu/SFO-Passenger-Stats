...
There is no suprise on the close to 50%-50% split on departure and arrival passengers in SFO. As the domestic carriers tend not to operate transiting flight via SFO, the portion of transit flights in SFO becomes significantly small. At the same time, there are transit flighs operated by some foreign carriers such as Singapore Airlines' `Singapore - Hong Kong - San Francisco` route and China Southern Airlines' `Canton - Wuhan - San Francisco` route. Since SFO is the final destination of either route, therefore passengers traveled with those routes do not count toward `transit` flights.

### II. Destination

As the bar chart on the monthly average passengers count shows that roughly 80% of the SFO passengers are domestic passengers. The map below shows the passengers count in North America between `2006` and `2017`.
...
And the map below shows the passengers count on other regions other than North America between `2006` and `2017`.
...
The chart shows that a large portion of SFO travelers traveling by `United Airlines` as SFO is one of the hub of `United Airlines` in the West Coast. `Southwest Airline`, `Virgin America`(Recently Merged with `Alaska Airlines`), `Delta Airlines`, `American Airlines` are the other top 5 major carrier in SFO in passenger count.
