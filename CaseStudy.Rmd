...
wd <- getwd()
if ("chunheisiu" %in% strsplit(wd, "/")[[1]]) {
...
data$isDomestic %<>%
  plyr::revalue(c("Domestic" = T, "International" = F))

## Reformat the dates into Date objects
data$date %<>% 
  as.character(stringAsFactors = F) %>% 
  as.yearmon("%Y%m") %>% 
  as.Date()

# Get min and max dates
year_max <- max(data$date) %>% format("%Y") %>% as.numeric()
year_min <- min(data$date) %>% format("%Y") %>% as.numeric()
